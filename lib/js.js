...
class JSRewriter {
    ...
    createHead(url) {
        ...
        if (!self.$corrosion && self.importScripts) {
            ...
            self.$corrosion = new Corrosion({ url: ${JSON.stringify(url)}, codec: '${this.ctx.config.codec || 'plain'}', serviceWorker: true,  window: self, prefix: '${this.ctx.prefix || '/service/'}', ws: ${this.ctx.config.ws || true}, cookies: ${this.ctx.config.cookies || false}, title: ${JSON.stringify(this.ctx.config.title || '')}, }); $corrosion.init();
        };\n`;
    };
    iterate(ast, handler) {
